...
The `home-routes` handler in the `vue-stocktrader-backend.routes.home` namespace
...
The `home-page` function will in turn call the `vue-stocktrader-backend.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `vue-stocktrader-backend.handler` namespace:
...
Request middleware functions are located under the `vue-stocktrader-backend.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `vue-stocktrader-backend.dev-middleware` namespace found in
